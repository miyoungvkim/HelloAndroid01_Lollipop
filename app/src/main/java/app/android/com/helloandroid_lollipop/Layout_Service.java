...
public class Layout_Service extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btn_service_start :
                startService(new Intent(Layout_Service.this, MyService.class)); // 서비스 시작
                break;

            case R.id.btn_service_stop :
                stopService(new Intent(Layout_Service.this, MyService.class)); // 서비스 종료
                break;

            case R.id.btn_service_bind :
                if (!isBind) //해당 액티비이에서 바운딩 중이 아닐때만 호출 - 바운딩 시작
                    bindService(new Intent(Layout_Service.this, MyService.class), sconn, BIND_AUTO_CREATE);
                break;
